...
#'
#' @param frequency_exceeding_random_child         A vector of frequency of exceeding corresponding to a random child
#' @param frequency_exceeding_high_consumer_child  A vector of frequency of exceeding corresponding to a high consumer child
#'
#' @return
#' a plot
#'
...
#' @export
#'
graph_pp_both <- function(frequency_exceeding_random_child, frequency_exceeding_high_consumer_child){

  # data wide format
  data_plot_wide <- data.frame(frequency_exceeding_random_child = sort(frequency_exceeding_random_child),
                               frequency_exceeding_high_consumer_child = sort(frequency_exceeding_high_consumer_child),
                               frequency_exceeding_cdf = c(1:length(frequency_exceeding_random_child)/length(frequency_exceeding_random_child)))

  # data long format
  data_plot_long <- gather(data_plot_wide, "group", "values", frequency_exceeding_random_child,
                      frequency_exceeding_high_consumer_child, factor_key = TRUE)
