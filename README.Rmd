...
## Example
This example does the aluminium exposure assessment by precise probability.
...
```{r fig1, out.width = '70%'}
 ...
 ## A high consumer child
  graph_pp(frequency_exceeding = TWI_pp_high_consumer$frequency_exceeding)
    
 ## both graphs
    graph_pp_both(frequency_exceeding_random_child = TWI_pp_random_child$frequency_exceeding, 
                   frequency_exceeding_high_consumer = TWI_pp_high_consumer$frequency_exceeding)
```

```{r fig2, out.width = '85%'}
 ## both graphs
    graph_pp_both(prob_exceed_all = TWI_pp_random_child$frequency_exceeding, 
                   prob_exceed_high_consumer = TWI_pp_high_consumer$frequency_exceeding)
```
Now, let us estimate the frequency of exceeding the safety threshold by bounded probability. 

First, let us consider a random child
...
We can visualize the results individualy or together. In this case, epistemic uncertainty has been characterized by a probabilty box.

Individual figures

```{r fig3, out.width = '70%'}
 ## All children
 graph_bp(lower_points = lower_bound_random_child$opt_prob$prob_exceed, 
          upper_points = upper_bound_random_child$opt_prob$prob_exceed)
...
```{r fig4, out.width = '70%'}  
 ## Both graphs
  graph_bp_both(lower_points_all = lower_bound_random_child$opt_prob$prob_exceed, 
                upper_points_all = upper_bound_random_child$opt_prob$prob_exceed,
